## create webhook ref: https://docs.microsoft.com/en-us/azure/automation/automation-webhooks

workflow ScaleAzureSQLDW {
    Param(
        [Parameter(Mandatory=$true)]
        [string]$ConnectionName = "AzureRunAsConnection",
        [parameter(Mandatory=$true)]
        [string]$SQLActionAccountName = "hiramdw",
        [parameter(Mandatory=$true)]
        [string]$ServerName = "hiramdw.database.windows.net",
        [parameter(Mandatory=$true)]
        [string]$DWName = "dw",
        [parameter(Mandatory=$true)]
        [ValidateSet(
        ...
        [string]$RequestedServiceObjectiveName = "DW200c",
        [int]$RetryCount = 5,
        [int]$RetryTime = 60  
    )
